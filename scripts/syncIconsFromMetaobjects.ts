...
async function main() {
  ...
  for (const page of seoPages) {
    if (page.seo?.jsonLd?.image) {
      assetsMap[page.slug] = page.seo.jsonLd.image;
  ...
  for (const col of collections) {
    if (col.image) {
      assetsMap[col.handle] = col.image;
